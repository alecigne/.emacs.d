...
=normal-top-level-add-subdirs-to-load-path= will add all subdirectories
of this lisp directory to "load-path".

The code is from [[http://emacswiki.org/emacs/LoadPath][here]]. Explanation:
